...
namespace FunctionsSdkE2ETests
{
    public class E2ETests
    {
        ...
        {
            string solutionName = "V1";

            string solutionFile = solutionName + ".sln";
            string workingDir = FindContainingDirectory(solutionFile);

            // build in a new path to ensure a clear nuget cache
            const string nugetEnvVar = "NUGET_PACKAGES";
            string packageDir = Path.Combine(Path.GetTempPath(), "FunctionsSdkTests", "tempNuget_" + Guid.NewGuid().ToString());
            string previousValue = Environment.GetEnvironmentVariable("NUGET_PACKAGES");

            try
            {
                Environment.SetEnvironmentVariable(nugetEnvVar, packageDir);
            ...
            {
                Environment.SetEnvironmentVariable(nugetEnvVar, previousValue);
        ...
        {
            RunTest("FrameworkVersions", 4);
        }

        [Fact]
        public void PublishRazor()
        {
            string publishDir = Path.Combine(Path.GetTempPath(), "FunctionsSdkTests", "pub_razor");
            if (Directory.Exists(publishDir))
            {
                Directory.Delete(publishDir, true);
            }
        ...
        [Fact]
        public void Build_NoTrailingSlashOnNugetPath()
        {
            const string nugetEnvVar = "NUGET_PACKAGES";
            string packageDir = Path.Combine(Path.GetTempPath(), "FunctionsSdkTests", "tempNuget").TrimEnd('\\');
            string previousValue = Environment.GetEnvironmentVariable("NUGET_PACKAGES");
